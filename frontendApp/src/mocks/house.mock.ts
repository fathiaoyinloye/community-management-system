...
import { addMockResidentAccount } from './auth.mock'
...
export async function mockRegisterHouse(payload: RegisterHousePayload): Promise<House> {
  ...
  if (payload.resident) {
    addMockResidentAccount(payload.resident, payload.resident.password)
  }
...
export async function mockUpdateHouse(id: string, updates: Partial<House>): Promise<House> {
  ...
  if (updates.resident) {
    addMockResidentAccount(updates.resident, updates.resident.password)
  }
